...
screenshot_alt: "@sebastienrousseau/jsdoc-config architecture and rule enforcement."
footer_note: "@sebastienrousseau/jsdoc-config is part of the @sebastienrousseau/config suite. Published under Apache-2.0 OR MIT."
copyright: "© 2026 Sebastien Rousseau. Licensed under Apache-2.0 OR MIT."
translation_key: "home"
title: "@sebastienrousseau/jsdoc-config — Shareable JSDoc configuration rules for standardized, searchable JavaScript API documentation."
description: "Shareable JSDoc configuration rules for standardized, searchable JavaScript API documentation."
keywords: "jsdoc-config, jsdoc, documentation generator, api docs, typedoc, javascript, developer tools, sebastien rousseau, standard"
hero_tag: "v0.0.6"
hero_tagline: "Standardized JSDoc configuration for automated API documentation"
headline: "@sebastienrousseau/jsdoc-config"
lead: "Shareable JSDoc configuration rules for standardized, searchable JavaScript API documentation."
cta_primary: "Install @sebastienrousseau/jsdoc-config"
...
snippet_caption: "Install @sebastienrousseau/jsdoc-config"
snippet_code: "npm install --save-dev @sebastienrousseau/jsdoc-config"
...
## What is @sebastienrousseau/jsdoc-config?

**@sebastienrousseau/jsdoc-config** is an enterprise-grade, shareable configuration for [JSDoc](https://github.com/sebastienrousseau/jsdoc-config)—the API documentation generator for JavaScript source files. Designed to deliver vetted, deterministic defaults, this package eliminates manual setup boilerplate and establishes rigorous code quality standards across modern software development pipelines.

By adopting this configuration, engineering teams ensure unified consistency across local development, continuous integration, and automated deployments. The package adheres strictly to the OpenSSF Best Practices badge program (ID: 14509) and achieves top-tier security ratings via Scorecard audits.
...
## Why Developers Choose @sebastienrousseau/jsdoc-config

Modern JavaScript and TypeScript repositories frequently suffer from configuration drift, fragmented standards, and maintenance overhead. **@sebastienrousseau/jsdoc-config** directly solves these challenges:
...
Integrating `@sebastienrousseau/jsdoc-config` into any new or existing repository requires only three simple steps.
...
### Step 2: Configure `jsdoc.json`

Create or update your `jsdoc.json` file at the root of your project to extend the shared configuration:
...
### Step 3: Run JSDoc in Your Workflow

Execute JSDoc directly or configure standard quality scripts in your `package.json`:

```bash
npx jsdoc -c jsdoc.json
```

Generate HTML API reference documentation from code annotations.
...
### Why use JSDoc when using TypeScript?
JSDoc tags enrich TypeScript declarations with rich contextual markdown, param descriptions, deprecation notices, and usage examples that render seamlessly in IDE hover cards.

### Can I override individual rules in my project?
Yes. Every configuration allows local overrides directly within your `jsdoc.json` file or inline settings. Detailed override recipes are documented in the [Usage Guide](usage/).

### Is @sebastienrousseau/jsdoc-config safe for production CI/CD?
